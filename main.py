...
def fault_DAG():
    for node in n1 :
        if node not in pred:
            if f.nodes[node]['gateType'] == "PrimaryInput" or node==node_fault :
                continue
            else:
                fault_update(node)
...
k=list(inputs)
k.sort()
#K contains all the nodes as a list, k=n1. Can be interchanged
k+=['Z']
...
#print(var2)
fobj=open("Example output.txt",'a+') #Writing to the output file

for i in range(len(var1)):
    if var1[i][-1]!=var2[i][-1]: #if the Z value differs, we include it as a valid tesst input vector
        a_s=var2[i][:4]
        a_l=[]
        F=1
        for ii in a_s:
...
fobj.close()
if F==0:
    print("NO INPUT TEST VECTOR CAN HELP US IDENTIFY THIS STUCK-AT-FAULT")
end = time.time()
